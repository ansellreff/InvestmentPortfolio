...
import {
  Search, Plus, Star, DollarSign, TrendingUp, LineChart,
  Clock, ChevronRight, X, Loader2, ChevronDown, BarChart3
} from 'lucide-react';
...
export default function StocksMarketPage() {
  ...
  const fetchStocks = async (append = false) => {
    ...
    try {
      const response = await fetch('/api/search?type=stock&limit=50');
      const result = await response.json();

      if (result.success && result.data) {
        const newStocks: Stock[] = result.data
          .filter((item: SearchResult) => {
            // Only include US stocks (not Indonesian, not crypto)
            if (item.type === 'CRYPTO') return false;
            if (item.symbol.endsWith('.JK')) return false;
            return item.type === 'STOCK' || item.type === 'ETF' || item.type === 'INDEX';
          })
  ...
  useEffect(() => {
    const timeoutId = setTimeout(async () => {
      ...
      setIsSearching(true);
      try {
        const response = await fetch(`/api/search?q=${encodeURIComponent(searchQuery)}&type=stock&limit=10`);
        const result = await response.json();

        if (result.success) {
          setSearchResults(result.data?.filter((r: SearchResult) => {
            if (r.type === 'CRYPTO') return false;
            if (r.symbol.endsWith('.JK')) return false;
            return true;
          }) || []);
          ...
          <div className="container mx-auto px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Link href="/profile">
                  <Button variant="secondary" size="sm" className="h-8 bg-white text-green-600 hover:bg-slate-100">
                    Profile
                  </Button>
                </Link>
                <Link href="/dashboard">
        ...
        <section className="mb-8">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-slate-600 dark:text-slate-400">
                    Real-time prices for major US companies
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="relative mt-6 max-w-2xl" ref={searchDropdownRef}>
            <div className="relative">
              ...
              <Input
                ref={searchInputRef}
                placeholder="Search US stocks (e.g., Apple, Tesla, AAPL, TSLA)..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onFocus={() => searchQuery.length >= 2 && setShowSearchDropdown(true)}
                className="pl-12 pr-10 h-14 text-base bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-green-500"
              />
              {searchQuery && (
                <button
                  onClick={() => {
                ...
                {isSearching ? (
                  ...
                  <div className="py-2">
                    {searchResults.map((result) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-lg bg-green-100 dark:bg-green-900/30 text-green-600 transition-transform group-hover:scale-110">
                            <BarChart3 className="h-4 w-4" />
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-slate-900 dark:text-white">{result.symbol}</span>
                              <Badge className="text-xs bg-green-100 text-green-600">US Stock</Badge>
                            </div>
                  ...
                  <div className="py-8 text-center px-4">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-500 mt-1">
                      Try the exact symbol (e.g., AAPL, GOOGL, TSLA)
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredStocks.map((stock) => {
                  ...
                  return (
                    <Card
                      key={stock.symbol}
                      className="group hover:shadow-xl transition-all duration-300 border-slate-200 dark:border-slate-700 hover:border-green-300 dark:hover:border-green-600"
                    >
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-4">
                          ...
                          <Badge className="text-xs bg-green-100 text-green-600">US</Badge>
                        </div>
