...
import {
  Search, Plus, Star, Bitcoin, TrendingUp, LineChart,
  Clock, ChevronRight, X, Loader2, ChevronDown, Sparkles
} from 'lucide-react';
...
export default function CryptoMarketPage() {
  ...
  const fetchCrypto = async (append = false) => {
    if (append) setLoadingMore(true);
    else setInitialLoading(true);

    try {
      const response = await fetch('/api/search?type=crypto&limit=50');
      const result = await response.json();

      if (result.success && result.data) {
        const newCryptos: Crypto[] = result.data
          .filter((item: SearchResult) => item.type === 'CRYPTO')
        ...
        // Sort by market cap rank (professional approach) - rank 1 = highest market cap
        const sorted = [...newCryptos].sort((a, b) => {
        ...
        if (append) {
          ...
          setCryptos(prev => {
            ...
            // If no new items, all cryptos are exhausted
            if (toAdd.length === 0) {
              setHasMore(false);
            ...
            setLoadedSymbols(new Set([...existingSymbols, ...toAdd.map(c => c.symbol)]));

            // Fetch prices for new items
            toAdd.forEach(crypto => fetchPrice(crypto));
          ...
          setCryptos(initialItems);
          setLoadedSymbols(new Set(initialItems.map(c => c.symbol)));

          // Fetch prices for initial items
          initialItems.forEach(crypto => fetchPrice(crypto));

          // Check if there's more data
          setHasMore(newCryptos.length > 12);
        }
  ...
  useEffect(() => {
    const timeoutId = setTimeout(async () => {
      ...
      setIsSearching(true);
      try {
        const response = await fetch(`/api/search?q=${encodeURIComponent(searchQuery)}&type=crypto&limit=10`);
        const result = await response.json();

        if (result.success) {
          setSearchResults(result.data?.filter((r: SearchResult) => r.type === 'CRYPTO') || []);
  ...
  // Fetch price for a crypto
  const fetchPrice = async (crypto: Crypto) => {
    if (loadingPrices.has(crypto.symbol)) return;

    setLoadingPrices(prev => new Set(prev).add(crypto.symbol));

    try {
      const response = await fetch(`/api/crypto/price?symbol=${crypto.symbol}`);
      const result = await response.json();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      cryptos.forEach(crypto => fetchPrice(crypto));
    }, 30000);
  ...
  const handleAddToComparison = (symbol: string, name: string) => {
    ...
    addInstrument({ symbol, name, type: 'CRYPTO', price, currency });
  };

  const handleAddToWatchlist = (symbol: string, name: string) => {
    addToWatchlist({ symbol, name, type: 'CRYPTO', addedAt: new Date().toISOString() });
  };
          ...
          <div className="container mx-auto px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Link href="/profile">
                  <Button variant="secondary" size="sm" className="h-8 bg-white text-orange-600 hover:bg-slate-100">
                    Profile
                  </Button>
                </Link>
                <Link href="/dashboard">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-slate-600 dark:text-slate-400">
                    Real-time prices for 15,000+ cryptocurrencies
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="relative mt-6 max-w-2xl" ref={searchDropdownRef}>
            <div className="relative">
              ...
              <Input
                ...
                className="pl-12 pr-10 h-14 text-base bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-orange-500"
              />
              {searchQuery && (
                <button
                  onClick={() => {
                ...
                {isSearching ? (
                  ...
                  <div className="py-2">
                    {searchResults.map((result) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="p-2 rounded-lg bg-orange-100 dark:bg-orange-900/30 text-orange-600 transition-transform group-hover:scale-110">
                            <Bitcoin className="h-4 w-4" />
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-slate-900 dark:text-white">{result.symbol}</span>
                              <Badge className="text-xs bg-orange-100 text-orange-600">Crypto</Badge>
                            </div>
                  ...
                  <div className="py-8 text-center px-4">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 font-medium">No cryptocurrencies found</p>
                    <p className="text-sm text-slate-500 dark:text-slate-500 mt-1">
                      Try the exact symbol (e.g., BTC-USD, ETH-USD)
                    </p>
                  </div>
                )}
              </div>
        ...
        <section>
          {initialLoading ? (
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredCryptos.map((crypto) => {
                  const priceData = stockPrices[crypto.symbol];
                  const isLoading = loadingPrices.has(crypto.symbol);
                  const isAdded = isSelected(crypto.symbol);
                  const isWatched = isInWatchlist(crypto.symbol);

                  return (
                    <Card
                      key={crypto.symbol}
                      className="group hover:shadow-xl transition-all duration-300 border-slate-200 dark:border-slate-700 hover:border-orange-300 dark:hover:border-orange-600"
                    >
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-bold text-slate-900 dark:text-white">{crypto.symbol}</h3>
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => handleAddToWatchlist(crypto.symbol, crypto.name)}
                          ...
                          <Button
                            ...
                            onClick={() => isAdded ? removeInstrument(crypto.symbol) : handleAddToComparison(crypto.symbol, crypto.name)}
                            className="flex-1"
                          >
                            {mounted && isAdded ? <>Added</> : <><Plus className="h-4 w-4 mr-1" />Add</>}
                          </Button>
                          <Link href={`/instrument/${crypto.symbol}`} className="flex-1">
            ...
            <Card className="p-12 text-center">
              <Bitcoin className="h-16 w-16 mx-auto text-slate-300 dark:text-slate-600 mb-4" />
