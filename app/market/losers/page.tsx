...
export default function LosersPage() {
  const [losers, setLosers] = useState<LoserData[]>([]);
  ...
  useEffect(() => {
    async function handleRefresh() {
      try {
        ...
        // Sort by change percent ascending (most negative first) and filter negative changes
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold">Top Losers</h1>
          </div>
          <p className="text-slate-600 dark:text-slate-400">
            Stocks with the highest price decreases today
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <Card className="p-6 bg-white dark:bg-slate-900 shadow-lg">
            <p className="text-slate-600 dark:text-slate-400 text-sm mb-1">Total Losers</p>
            <p className="text-3xl font-bold text-red-600">{losers.length}</p>
          </Card>
          <Card className="p-6 bg-white dark:bg-slate-900 shadow-lg">
            <p className="text-slate-600 dark:text-slate-400 text-sm mb-1">Avg. Loss</p>
            <p className="text-3xl font-bold">
              {losers.length > 0
                ? `${(losers.reduce((sum, l) => sum + l.changePercent, 0) / losers.length).toFixed(2)}%`
                : '0.00%'}
            </p>
          </Card>
          <Card className="p-6 bg-white dark:bg-slate-900 shadow-lg">
            <p className="text-slate-600 dark:text-slate-400 text-sm mb-1">Worst Performer</p>
            <p className="text-3xl font-bold">
              {losers.length > 0 ? `${losers[0].changePercent.toFixed(2)}%` : '0.00%'}
            </p>
          </Card>
        </div>
                ...
                {losers.length === 0 ? (
                  ...
                  losers.map((loser, index) => (
                    ...
                    >
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="text-lg font-bold">{loser.symbol}</span>
                          {index === 0 && (
                            <span className="px-2 py-1 bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-100 text-xs rounded-full font-medium">
                              #1
                            </span>
                          )}
                        </div>
                      ...
                      <td className="p-4 text-right font-semibold">
                        {formatPriceInCurrency(loser.price, loser.currency)}
                      </td>
                      <td className="p-4 text-right text-red-600 font-semibold">
                        {loser.change.toFixed(2)}
                      </td>
                      <td className="p-4 text-right">
                        <span className="px-3 py-1 bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-100 rounded-full font-semibold">
                      ...
                      <td className="p-4 text-right text-slate-600 dark:text-slate-400">
                        {formatMarketCap(loser.marketCap)}
                      </td>
                      <td className="p-4 text-center">
                        <Link href={`/instrument/${loser.symbol}`}>
        ...
        <Card className="mt-6 p-4 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-900">
          <p className="text-sm text-blue-800 dark:text-blue-200">
            <strong>Note:</strong> Data is updated in real-time. Top losers are based on daily percentage change.
            This page shows stocks with negative price movement only.
          </p>
        </Card>
      </div>
    </div>
